...
import { createIndexedDbAvatarCreatorStorage } from './indexeddb-avatar-creator-storage.js';
...
test('creates the Avatar store and overwrites the fixed current record', async () => {
  ...
  const storage = createIndexedDbAvatarCreatorStorage(indexedDB);
  ...
  const reopened = createIndexedDbAvatarCreatorStorage(indexedDB);
